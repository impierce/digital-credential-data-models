...
fn test_achievement_credential() {
    ...
    let json_v_achievement_cred: serde_json::Value = serde_json::to_value(&json_achievement_credential).expect("to_value");
    let json_str_achievement_cred = serde_json::to_string_pretty(&json_achievement_credential).expect("str");
    
    let file = File::open("src/tests/json_examples/achievement_credential.json").expect("Failed to open file");
    ...
    let mut file2 = File::open("src/tests/json_examples/achievement_credential.json").expect("Failed to open file");
